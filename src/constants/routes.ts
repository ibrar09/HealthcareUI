/**
 * Centralized path strings — modules import these instead of typing
 * "/doctor/dashboard" by hand in multiple places. Update a path here,
 * it updates everywhere that links to it.
 */
export const ROUTES = {
  DOCTOR: {
    SIGN_IN: "/doctor/sign-in",
    DASHBOARD: "/doctor/dashboard",
    PATIENTS: "/doctor/patients",
    PATIENT_DETAIL: (id: string) => `/doctor/patients/${id}`,
    ENCOUNTER: (patientId: string) => `/doctor/encounter/${patientId}`,
    APPOINTMENTS: "/doctor/appointments",
    QUEUE: "/doctor/queue",
    REQUESTS: "/doctor/requests",
    PAYMENT: "/doctor/payment",
    PRODUCT_STOCK: "/doctor/product-stock",
    HELP_CENTER: "/doctor/help-center",
    SETTINGS: "/doctor/settings",
    REPORT: "/doctor/report",
    SCHEDULE: "/doctor/schedule",
    MESSAGES: "/doctor/messages",
  },
  HOSPITAL_ADMIN: {
    SIGN_IN: "/hospital-admin/sign-in",
    DASHBOARD: "/hospital-admin/dashboard",
    RECEPTION: "/hospital-admin/reception",
    FACILITIES: "/hospital-admin/facilities",
    STAFF: "/hospital-admin/staff",
    STAFF_DETAIL: (id: string) => `/hospital-admin/staff/${id}`,
    PATIENTS: "/hospital-admin/patients",
    PATIENT_DETAIL: (id: string) => `/hospital-admin/patients/${id}`,
    MPI_REVIEW: "/hospital-admin/patients/mpi-review",
    MPI_REVIEW_PAIR: (id: string) => `/hospital-admin/patients/mpi-review/${id}`,
    BEDS: "/hospital-admin/beds",
    APPOINTMENTS: "/hospital-admin/appointments",
    BILLING: "/hospital-admin/billing",
    EMERGENCY: "/hospital-admin/emergency",
    LABORATORY: "/hospital-admin/laboratory",
    RADIOLOGY: "/hospital-admin/radiology",
    PHARMACY: "/hospital-admin/pharmacy",
    OPERATION_THEATRE: "/hospital-admin/operation-theatre",
    INVENTORY: "/hospital-admin/inventory",
    REPORTS: "/hospital-admin/reports",
    AUDIT: "/hospital-admin/audit",
    CONFIGURATION: "/hospital-admin/configuration",
    ALERTS: "/hospital-admin/alerts",
  },
  LABORATORY: {
    SIGN_IN: "/laboratory/sign-in",
    DASHBOARD: "/laboratory/dashboard",
    ORDERS: "/laboratory/orders",
    COLLECTION: "/laboratory/collection",
    PROCESSING: "/laboratory/processing",
    WORKLISTS: "/laboratory/worklists",
    CRITICAL_RESULTS: "/laboratory/critical-results",
    TEST_CATALOG: "/laboratory/tests",
    ANALYZERS: "/laboratory/analyzers",
    QUALITY_CONTROL: "/laboratory/quality-control",
    REPORTS: "/laboratory/reports",
    REJECTED: "/laboratory/rejected",
    INVENTORY: "/laboratory/inventory",
    BILLING: "/laboratory/billing",
    NOTIFICATIONS: "/laboratory/notifications",
    ANALYTICS: "/laboratory/analytics",
    AUDIT: "/laboratory/audit",
    CONFIGURATION: "/laboratory/configuration",
  },
  RADIOLOGY: {
    SIGN_IN: "/radiology/sign-in",
    DASHBOARD: "/radiology/dashboard",
    ORDERS: "/radiology/orders",
    TECHNICIAN_WORKLIST: "/radiology/technician-worklist",
    RADIOLOGIST_WORKLIST: "/radiology/radiologist-worklist",
    STUDY_WORKSPACE: (orderId: string) => `/radiology/orders/${orderId}/workspace`,
    CRITICAL_FINDINGS: "/radiology/critical-findings",
    PEER_REVIEW: "/radiology/peer-review",
    REFERRALS: "/radiology/referrals",
    PROCEDURES: "/radiology/procedures",
    EQUIPMENT: "/radiology/equipment",
    BILLING: "/radiology/billing",
    REPORTS_ANALYTICS: "/radiology/reports-analytics",
    NOTIFICATIONS: "/radiology/notifications",
    AUDIT: "/radiology/audit",
    CONFIGURATION: "/radiology/configuration",
  },
  PHARMACY: {
    SIGN_IN: "/pharmacy/sign-in",
    DASHBOARD: "/pharmacy/dashboard",
    PRESCRIPTION_QUEUE: "/pharmacy/prescription-queue",
    VERIFY_ORDER: (id: string) => `/pharmacy/orders/${id}/verify`,
    DISPENSING: "/pharmacy/dispensing",
    IV_COMPOUNDING: "/pharmacy/iv-compounding",
    PATIENT_360: "/pharmacy/patient-360",
    RECONCILIATION: "/pharmacy/reconciliation",
    INTERVENTIONS: "/pharmacy/interventions",
    ADVERSE_REACTIONS: "/pharmacy/adverse-reactions",
    FORMULARY: "/pharmacy/formulary",
    INVENTORY: "/pharmacy/inventory",
    CONTROLLED_MEDICATIONS: "/pharmacy/controlled-medications",
    RECALLS: "/pharmacy/recalls",
    PROCUREMENT: "/pharmacy/procurement",
    ALERTS: "/pharmacy/alerts",
    REPORTS: "/pharmacy/reports",
    AUDIT: "/pharmacy/audit",
    CONFIGURATION: "/pharmacy/configuration",
  },
  NURSING: {
    SIGN_IN: "/nursing/sign-in",
    DASHBOARD: "/nursing/dashboard",
    PATIENTS: "/nursing/patients",
    PATIENT_DETAIL: (id: string) => `/nursing/patients/${id}`,
    WARD: "/nursing/ward",
    MEDICATION_ADMINISTRATION: "/nursing/medication-administration",
    ASSESSMENTS: "/nursing/assessments",
    PATIENT_ASSESSMENT: (id: string) => `/nursing/patients/${id}/assessment`,
    TASKS: "/nursing/tasks",
    NURSING_NOTES: "/nursing/notes",
    PATIENT_NOTES: (id: string) => `/nursing/patients/${id}/notes`,
    CARE_PLANS: "/nursing/care-plans",
    PATIENT_CARE_PLANS: (id: string) => `/nursing/patients/${id}/care-plans`,
    ALERTS: "/nursing/alerts",
    SHIFT_HANDOVER: "/nursing/shift-handover",
    DISCHARGE: "/nursing/discharge",
    ROUNDS: "/nursing/rounds",
    VITALS: "/nursing/vitals",
    ORDERS_RESULTS: "/nursing/orders-results",
    SPECIMEN_COLLECTION: "/nursing/specimen-collection",
    SAFETY: "/nursing/safety",
    PROCEDURES: "/nursing/procedures",
    BED_MANAGEMENT: "/nursing/bed-management",
    TRANSFERS: "/nursing/transfers",
    CARE_TEAM: "/nursing/care-team",
    DOCUMENTS: "/nursing/documents",
    MESSAGES: "/nursing/messages",
    NOTIFICATIONS: "/nursing/notifications",
    REPORTS: "/nursing/reports",
    SETTINGS: "/nursing/settings",
  },
  EMERGENCY: {
    SIGN_IN: "/emergency/sign-in",
    DASHBOARD: "/emergency/dashboard",
    ARRIVALS: "/emergency/arrivals",
    TRIAGE: "/emergency/triage",
    QUEUE: "/emergency/queue",
    BEDS: "/emergency/beds",
    DOCTOR_WORKSPACE: "/emergency/doctor-workspace",
    NURSING_WORKSPACE: "/emergency/nursing-workspace",
    ENCOUNTER: (id: string) => `/emergency/encounters/${id}`,
    ORDERS: "/emergency/orders",
    PROCEDURES: "/emergency/procedures",
    MONITORING: "/emergency/monitoring",
    CRITICAL_RESULTS: "/emergency/critical-results",
    CONSULTATIONS: "/emergency/consultations",
    DISPOSITION: "/emergency/disposition",
    AMBULANCE: "/emergency/ambulance",
    HANDOVER: "/emergency/handover",
    REPORTS: "/emergency/reports",
    AUDIT: "/emergency/audit",
    CONFIGURATION: "/emergency/configuration",
  },
  BILLING: {
    SIGN_IN: "/billing/sign-in",
  },
  PLATFORM_ADMIN: {
    SIGN_IN: "/platform-admin/sign-in",
  },
  PATIENT_APP: {
    HOME: "/app/home",
  },
} as const;
