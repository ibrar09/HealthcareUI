...
import type { LabAuditEvent } from "@modules/hospital-admin/api";

interface LabAuditLogPanelProps {
  events: LabAuditEvent[];
  actionFilter: string;
  onActionFilterChange: (value: string) => void;
  actionOptions: string[];
}

/** Module-local — Laboratory "Audit" tab: hospital-wide, filterable trail of every administrative action this section can take (order capture/cancel, critical acknowledgment). */
export function LabAuditLogPanel({ events, actionFilter, onActionFilterChange, actionOptions }: LabAuditLogPanelProps) {
  return (
              ...
              <div key={e.id} className="flex items-center justify-between gap-3 rounded-xl border border-line px-3.5 py-2.5">
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs font-mono text-on-surface-variant">{e.orderNumber}</span>
                  </div>
