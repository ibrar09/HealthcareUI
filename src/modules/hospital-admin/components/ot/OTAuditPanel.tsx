...
import type { OTAuditEntry, OTAuditEntityType } from "@modules/hospital-admin/api";

const entityTypes: OTAuditEntityType[] = ["case", "room", "procedure", "instrument", "consumable", "implant", "specimen", "equipment"];

const entityLabels: Record<OTAuditEntityType, string> = {
  case: "Case",
  room: "Room",
  procedure: "Procedure",
...
interface OTAuditPanelProps {
  entries: OTAuditEntry[];
  search: string;
  onSearchChange: (value: string) => void;
  entityFilter: OTAuditEntityType | "all";
  onEntityFilterChange: (value: OTAuditEntityType | "all") => void;
}

/** Module-local — OT Audit Log (spec §40): every workflow mutation this section owns, logged from day one — never clinical content. */
export function OTAuditPanel({ entries, search, onSearchChange, entityFilter, onEntityFilterChange }: OTAuditPanelProps) {
  return (
    <div className="pb-8">
      <Card hero>
        <div className="flex items-center justify-between mb-4 gap-3 flex-wrap">
          ...
          <div className="flex items-center gap-3 flex-wrap">
            <select className="rounded-input border border-line px-3 py-2 text-sm outline-none focus:border-signal-indigo" value={entityFilter} onChange={(e) => onEntityFilterChange(e.target.value as OTAuditEntityType | "all")}>
