...
// "Fatima Khalid" deliberately isn't reused from any existing patient name
// in this mock world (Fatima Sheikh, Noor Fatima are patients) — staff and
// patient identities stay distinct even when a first name overlaps.
const MOCK_NURSE_ACCOUNT = { email: "fatima.khalid@citygeneral.org", name: "Nurse Fatima Khalid", ward: "Medical Ward A", hospital: "City General Hospital" };

const emailPattern = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;

export function NurseSignIn() {
  const navigate = useNavigate();
  const { signIn } = useAuth();

  const [email, setEmail] = useState(MOCK_NURSE_ACCOUNT.email);
  ...
  function validate(): boolean {
    ...
    if (!email.trim()) errors.email = "Nurse ID or email is required.";
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    if (email.trim().toLowerCase() !== MOCK_NURSE_ACCOUNT.email) {
      setSubmitting(false);
      setFormError("We couldn't verify those credentials. Check your Nurse ID/email and password, or contact IT Support.");
      return;
    }

    signIn({ name: MOCK_NURSE_ACCOUNT.name, role: ROLES.NURSE, hospital: MOCK_NURSE_ACCOUNT.hospital }, "mock-jwt-token");
    navigate(ROUTES.NURSING.DASHBOARD);
  }

  return (
    <AuthLayout
      brand="City General Hospital"
      tagline="Nursing Workspace — Medical Ward A"
      stats={["8 Wards", "312 Beds", "94% Occupancy"]}
    >
      <form className="w-full max-w-sm" onSubmit={handleSubmit} noValidate>
        <h2 className="font-display font-bold text-2xl text-on-surface mb-1">Nurse Sign In</h2>
        <p className="text-sm text-on-surface-variant mb-8">Enter your credentials to access your patient assignment.</p>
        ...
        <label htmlFor="nurse-email" className="block text-xs font-display font-semibold text-on-surface-variant mb-1.5">
          Nurse ID or Email
        </label>
        <input
          id="nurse-email"
          ...
          aria-describedby={fieldErrors.email ? "nurse-email-error" : undefined}
        ...
        />
        {fieldErrors.email && (
          <p id="nurse-email-error" className="mt-1.5 text-xs text-pulse-coral">
            {fieldErrors.email}
          </p>
        )}

        <label htmlFor="nurse-password" className="block text-xs font-display font-semibold text-on-surface-variant mb-1.5 mt-4">
          Password
        </label>
        <div className="relative">
          <input
            ...
            aria-describedby={fieldErrors.password ? "nurse-password-error" : undefined}
        ...
        {fieldErrors.password && (
          <p id="nurse-password-error" className="mt-1.5 text-xs text-pulse-coral">
            {fieldErrors.password}
          </p>
        )}
